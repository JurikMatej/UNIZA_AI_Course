import libs_env.blackbox.black_box_trial as bb_trial
import json
...
class BlackBoxMatch:
    ...
    def __init__(self, config_file_name):
        ...
        self.render              = self.json_config["render"]
    ...
    def run(self):
        ...
        for j in range(0, len(bots)):
            ...
            for i in range(0, self.trials_count):
                trial = bb_trial.BlackBoxTrial(bot_file_name, self.training_iterations, self.testing_iterations, self.seed + i, self.verbose, self.render)
                trial.train()
                trial.test()
